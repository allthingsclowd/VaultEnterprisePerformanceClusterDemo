...
which lxd &>/dev/null || {
  ...
  apt-get update
  cat conf/selection.1.conf | debconf-set-selections
  ...
  cp conf/lxd-bridge.1 /etc/default/lxd-bridge
  cp conf/dns.1.conf /etc/default/dns.conf
  service lxd restart
...
for s in consul{1..3}; do
  lxc info ${s} &>/dev/null || {
    ...
    cp conf/consul.d.1/server.hcl /var/lib/lxd/containers/${s}/rootfs/etc/consul.d
...
consul_client(){
  ...
  mkdir -p /var/lib/lxd/containers/${s}/rootfs/etc/consul.d
  cp conf/consul.d.1/client.hcl /var/lib/lxd/containers/${s}/rootfs/etc/consul.d
  cp conf/consul.service /var/lib/lxd/containers/${s}/rootfs/etc/systemd/system
  lxc exec ${s} -- bash /var/tmp/consul.sh
}
...
for s in vault{1..3}; do
  lxc info ${s} &>/dev/null || {
    ...
    lxc exec ${s} -- bash /var/tmp/vault.1.sh
  }
done
...
cp conf/haproxy.1.cfg /etc/haproxy/haproxy.cfg
